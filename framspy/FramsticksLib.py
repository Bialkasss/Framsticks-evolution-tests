...
class FramsticksLib:
	...
	def __init__(self, frams_path, frams_lib_name, sim_settings_files, home_dir=None):
		self.dissim_measure_density_distribution = None  # will be initialized only when necessary (for rare dissimilarity methods)

		init_args = [frams_path]
		if home_dir is not None:
			init_args.extend(["-d" + home_dir, "-D" + os.path.join(frams_path, "data")])
		if frams_lib_name is None:
			frams.init(*init_args)
		else:
			frams.init(*init_args, "-L" + frams_lib_name)
