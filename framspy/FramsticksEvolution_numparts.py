...
def frams_evaluate(frams_lib, individual):
	...
	try:
		...
		for crit in OPTIMIZATION_CRITERIA:
			val = default_evaluation_data[crit]
			if PRINT_EVALUATION_DATA:
				print("Genotype: %s | Full data: %s" % (genotype, default_evaluation_data), flush=True)
			if crit == "vertpos" and val <= 0.0:
				val -= default_evaluation_data["numparts"] * 0.0001
			fitness.append(val)
