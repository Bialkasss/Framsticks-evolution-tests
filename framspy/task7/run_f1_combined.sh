...
cleanup_and_notify() {
    ...
    if [ "$exit_code" -eq 0 ]; then
        echo "Combined Task 7 F1 experiment finished successfully!"
        play_success
    else
        echo "Combined Task 7 F1 experiment failed or was aborted with exit code $exit_code!"
        play_error
    fi
}
...
HALL_OF_FAME="${HALL_OF_FAME:-task7-final-f1-hof.gen}"
OUTPUT_PREFIX="${OUTPUT_PREFIX:-task7-final-f1}"
...
echo "========== STARTING ${RUNS} COMBINED TASK 7 F1 RUNS =========="
...
echo "All combined Task 7 F1 runs completed."
